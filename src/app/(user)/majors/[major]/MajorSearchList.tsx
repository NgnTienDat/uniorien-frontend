      ...
      <table className="w-full border-collapse bg-white shadow-sm">
        <thead>
          <tr className="bg-blue-100">
            <th className="border border-gray-300 px-3 py-1 text-center font-semibold text-gray-700">
              STT
            </th>
            <th className="border border-gray-300 px-3 py-1 text-center font-semibold text-gray-700">
              Tên trường
            ...
            {uniqueYears.map(year => (
              <th
                key={year}
                className="border border-gray-300 px-3 py-1 text-center font-semibold text-gray-700"
              >
                Điểm chuẩn {year}
              </th>
            ))}
              ...
              <tr key={`${universityIndex}-${majorIndex}`} className="hover:bg-gray-50">
                {majorIndex === 0 && (
                  <>
                    ...
                    <td
                      className="border border-gray-300 px-3 py-1 font-medium text-blue-600"
                      rowSpan={getUniversityRowSpan(university.majorDetails)}
                    >
